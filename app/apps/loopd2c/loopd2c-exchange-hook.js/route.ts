...
export async function GET(request: NextRequest) {
  try {
    ...
    const bootstrap = `window.MEGASKA_SHOP_DOMAIN=window.MEGASKA_SHOP_DOMAIN||${JSON.stringify(shop.shopDomain)};window.MEGASKA_API_BASE=window.MEGASKA_API_BASE||"/apps/loopd2c/api";\n`;
