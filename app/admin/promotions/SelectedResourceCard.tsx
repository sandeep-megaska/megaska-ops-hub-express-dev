...
export default function SelectedResourceCard({ resource, onRemove }: { resource: SelectedResource; onRemove: () => void }) {
  return <div className="flex items-center gap-3 rounded-lg border border-gray-200 bg-white p-3">
    ...
    <div className="min-w-0 flex-1"><p className="truncate text-sm font-medium text-gray-900">{resource.title}</p>{resource.handle ? <p className="truncate text-xs text-gray-500">{resource.handle}</p> : null}<p className="text-xs text-gray-500">{resource.resourceType === "PRODUCT" ? "Product" : "Collection"}</p></div>
    <button type="button" onClick={onRemove} className="rounded-md border px-2 py-1 text-xs text-gray-700">Remove</button>
  </div>;
}
