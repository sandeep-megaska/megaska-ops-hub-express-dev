...
export default function PromotionResourcePickerField({ kind, selections, onChange, disabled }: { kind: "product-trigger" | "collection-trigger" | "offer-product"; selections: SelectedResource[]; onChange: (next: SelectedResource[]) => void; disabled?: boolean }) {
  ...
  return <section className="space-y-3 rounded-lg border border-gray-200 p-4">
    <div className="flex items-center justify-between gap-3"><div><h3 className="text-sm font-semibold text-gray-900">{title}</h3><p className="text-xs text-gray-500">{isOffer ? "Select one Shopify product. Variants are ignored." : "Select Shopify resources for this trigger."}</p></div><button type="button" disabled={disabled || !pickerAvailable || loading} onClick={openPicker} className="rounded-lg bg-blue-600 px-3 py-2 text-sm font-semibold text-white disabled:bg-gray-300">{loading ? "Opening…" : isOffer && selections.length ? "Change product" : "Select"}</button>{isOffer && selections.length ? <button type="button" disabled={disabled} onClick={() => onChange([])} className="rounded-lg border border-red-300 px-3 py-2 text-sm font-semibold text-red-700 disabled:text-gray-300">Remove product</button> : null}</div>
    {initializing ? <p className="rounded-lg bg-gray-50 p-3 text-sm text-gray-600">Connecting to Shopify Admin…</p> : !pickerAvailable ? <p className="rounded-lg bg-gray-50 p-3 text-sm text-gray-600">Shopify Resource Picker is unavailable. Open this page from the Shopify Admin app.</p> : null}
