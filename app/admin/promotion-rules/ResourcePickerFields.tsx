...
export function ResourcePickerFields({ triggerType: initialTriggerType, triggerValue, triggerProduct, triggerCollection, triggerVariant, offerProduct, shopPresent, hostPresent }: { triggerType: string; triggerValue: string; triggerProduct: ResourceMeta; triggerCollection: ResourceMeta; triggerVariant: ResourceMeta; offerProduct: ResourceMeta; shopPresent: boolean; hostPresent: boolean }) {
  ...
  async function pick(type: ResourceType, purpose: "trigger" | "offer") {
    ...
    if (type === "product" && purpose === "offer") {
      ...
      const pickedSelectedVariantId = picked.selectedVariant?.id || picked.variant?.id || offer.variantGid || "";
      ...
      const extracted = pickedSelectedVariantId || nextVariants.length === 1 ? extractPickerVariantPrices(picked, pickedSelectedVariantId) : { variantPrice: null, variantCompareAtPrice: null, priceSource: "unavailable" as const, compareAtPriceSource: "unavailable" as const };
      const selectedVariant = nextVariants.find((variant) => variant.gid === pickedSelectedVariantId) || (nextVariants.length === 1 ? nextVariants[0] : nextVariants.find((variant) => variant.gid === offer.variantGid));
      ...
      setOffer({ ...nextProduct, variantGid: selectedVariant?.gid || pickedSelectedVariantId || "", variantTitle: selectedVariant?.title || picked.selectedVariant?.title || picked.variant?.title || "", variantPrice: selectedVariant?.variantPrice || extracted.variantPrice || "", variantCompareAtPrice: selectedVariant?.variantCompareAtPrice || extracted.variantCompareAtPrice || "", priceSource: selectedVariant?.priceSource || extracted.priceSource, compareAtPriceSource: selectedVariant?.compareAtPriceSource || extracted.compareAtPriceSource, imageUrl: nextProduct.imageUrl || selectedVariant?.imageUrl || "", image: nextProduct.image || selectedVariant?.image || "" });
    }
  }

  const variantOptions = useMemo(() => variants.length ? variants : offer.variantGid ? [{ gid: offer.variantGid, title: offer.variantTitle || "Selected variant", imageUrl: "", image: "", handle: "", resourceType: "variant" as const, variantPrice: offer.variantPrice || "", variantCompareAtPrice: offer.variantCompareAtPrice || "", priceSource: offer.priceSource || "unavailable", compareAtPriceSource: offer.compareAtPriceSource || "unavailable" }] : [], [variants, offer.variantGid, offer.variantTitle, offer.variantPrice, offer.variantCompareAtPrice, offer.priceSource, offer.compareAtPriceSource]);
