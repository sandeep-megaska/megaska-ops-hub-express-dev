...
type ResourceMeta = { id?: string; gid: string; title: string; image?: string | null; imageUrl: string | null; handle: string; resourceType?: ResourceType; variantGid?: string; variantTitle?: string; variantPrice?: string; variantCompareAtPrice?: string };
type ShopifyPickerVariant = { id?: string; title?: string; displayName?: string; handle?: string; price?: string | number; compareAtPrice?: string | number; image?: { url?: string; originalSrc?: string } };
...
function moneyText(value: unknown) { return typeof value === "number" ? String(value) : typeof value === "string" ? value.trim() : ""; }
function normalizeVariant(variant?: ShopifyPickerVariant): ResourceMeta {
  ...
  return { id: variant?.id || "", gid: variant?.id || "", title: variant?.title || variant?.displayName || "", image, imageUrl: image, handle: variant?.handle || "", resourceType: "variant", variantPrice: moneyText(variant?.price), variantCompareAtPrice: moneyText(variant?.compareAtPrice) };
}
...
export function ResourcePickerFields({ triggerType: initialTriggerType, triggerValue, triggerProduct, triggerCollection, triggerVariant, offerProduct, shopPresent, hostPresent }: { triggerType: string; triggerValue: string; triggerProduct: ResourceMeta; triggerCollection: ResourceMeta; triggerVariant: ResourceMeta; offerProduct: ResourceMeta; shopPresent: boolean; hostPresent: boolean }) {
  ...
  async function pick(type: ResourceType, purpose: "trigger" | "offer") {
    ...
    if (type === "product" && purpose === "offer") {
      ...
      setOffer({ ...nextProduct, variantGid: selectedVariant?.gid || "", variantTitle: selectedVariant?.title || "", variantPrice: selectedVariant?.variantPrice || "", variantCompareAtPrice: selectedVariant?.variantCompareAtPrice || "", imageUrl: nextProduct.imageUrl || selectedVariant?.imageUrl || "", image: nextProduct.image || selectedVariant?.image || "" });
    }
  }

  const variantOptions = useMemo(() => variants.length ? variants : offer.variantGid ? [{ gid: offer.variantGid, title: offer.variantTitle || "Selected variant", imageUrl: "", image: "", handle: "", resourceType: "variant" as const, variantPrice: offer.variantPrice || "", variantCompareAtPrice: offer.variantCompareAtPrice || "" }] : [], [variants, offer.variantGid, offer.variantTitle, offer.variantPrice, offer.variantCompareAtPrice]);
