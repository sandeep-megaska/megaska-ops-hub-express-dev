...
import { getPromotionRulesConfig, normalizePromotionRule, PROMOTION_RULES_CONFIG_MODULE_KEY, savePromotionRulesConfig, type PromotionConflictStrategy } from "../../../services/promotion-rules/config";
  ...
  try {
    ...
    if (intent === "upsert") {
      const rule = normalizePromotionRule({
        ...
        eligibility: { match: getString(formData, "match"), triggers: [{ type: getString(formData, "triggerType"), value: getString(formData, "triggerValue") }] },
        reward: { type: "offer_product", productGid: getString(formData, "offerProductGid"), variantGid: getString(formData, "offerVariantGid"), quantity: getString(formData, "quantity"), requiresDiscountEnforcement: getBool(formData, "requiresDiscountEnforcement") },
  ...
  return <main className="mx-auto max-w-6xl px-6 py-8">
    ...
    <form action={action} className="grid gap-6">
      ...
      <section className={`${cardClass} grid gap-5`}><h2 className="text-lg font-semibold text-gray-950">{config.rules.some((rule) => rule.id === selected.id) ? "Edit rule" : "Create rule"}</h2><div className="rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm font-medium text-amber-900">Display price is not yet enforced as a real checkout discount. Discount enforcement will be added in a later Shopify-safe phase. Hidden discounted products are not recommended. Offer product must be available in storefront for runtime display later.</div>
        ...
        <div className="grid gap-4 md:grid-cols-3"><Select label="Match mode" name="match" defaultValue={selected.eligibility.match}><option value="all">All</option><option value="any">Any</option></Select><Select label="Trigger type" name="triggerType" defaultValue={trigger.type}><option value="always">Always</option><option value="cart_contains_product">Cart contains product</option><option value="cart_contains_collection">Cart contains collection</option><option value="cart_contains_product_type">Cart contains product type</option><option value="cart_contains_tag">Cart contains tag</option><option value="cart_subtotal_gte">Cart subtotal greater than or equal to</option><option value="cart_quantity_gte">Cart quantity greater than or equal to</option></Select><Field label="Trigger value" name="triggerValue" defaultValue={trigger.value || ""} help="Use product/collection GID, product type, tag, subtotal, or quantity based on trigger type." /></div>
        <div className="grid gap-4 md:grid-cols-3"><Field label="Offer product GID / ID" name="offerProductGid" defaultValue={selected.reward.productGid} /><Field label="Offer variant GID / ID" name="offerVariantGid" defaultValue={selected.reward.variantGid} /><Field label="Quantity" name="quantity" type="number" defaultValue={selected.reward.quantity} /><Field label="Display price" name="offerPriceDisplay" defaultValue={selected.display.offerPriceDisplay} /><Field label="Compare price" name="comparePriceDisplay" defaultValue={selected.display.comparePriceDisplay} /><Check label="Requires discount enforcement warning" name="requiresDiscountEnforcement" defaultChecked={selected.reward.requiresDiscountEnforcement} /></div>
