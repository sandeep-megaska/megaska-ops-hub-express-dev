...
export async function PATCH(req: NextRequest, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    const approvalAmount = normalizedNextStatus === "APPROVED"
      ? parseRefundAmountMinor(body?.refundAmountRupees ?? body?.refundAmount ?? body?.amount)
        ...
        return NextResponse.json(
          {
            ...
            validation: {
              ...
              missingRefundAmount: !minorProvided && !body?.refundAmountRupees && !body?.refundAmount && !body?.amount,
            },
          },
          { status: 422 }
        );
    ...
    if (resolved?.ok) {
      if (normalizedNextStatus === "RETURN_RECEIVED" && resolved.refundRequestId) {
        ...
        if (
          ...
          console.info("[ISSUE REFUND RECOVERY] settled", {
            issueId: updated.id,
            resolvedShopId: resolved.resolvedShopId || null,
            resolvedOrderId: resolved.resolvedOrderId || null,
            resolvedAmount: resolved.resolvedAmount || null,
            resolvedPaymentMethod: resolved.resolvedPaymentMethod || null,
            refundRequestId: refund.id,
            walletTransactionId: settlement.walletTransaction.id,
            skipReason: null,
          });
