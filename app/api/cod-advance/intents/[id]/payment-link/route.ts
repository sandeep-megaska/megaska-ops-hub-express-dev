...
export async function POST(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const link = await createCodAdvancePaymentLink({ shopId: intent.shopId, intentId: intent.id, amountPaise: intent.advanceAmountPaise, currency: intent.currency, customerName: intent.customerProfile?.fullName, customerEmail: intent.customerProfile?.email, customerPhone: intent.customerProfile?.phoneE164 });
