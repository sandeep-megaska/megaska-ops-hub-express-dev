...
export function ExpressCheckoutSettingsForm() {
  ...
  return <form onSubmit={submit} className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm space-y-4">
    <div><h2 className="text-base font-semibold text-gray-900">Express checkout COD settings</h2><p className="text-sm text-gray-600">COD charges are disabled. Configure COD/refund copy shown in the checkout modal.</p></div>
    {error ? <p className="rounded-xl bg-red-50 px-3 py-2 text-sm text-red-700">{error}</p> : null}
    {message ? <p className="rounded-xl bg-green-50 px-3 py-2 text-sm text-green-700">{message}</p> : null}
    <label className="block text-sm text-gray-700">COD charge (₹)<input className="mt-1 w-full rounded-xl border border-gray-300 px-3 py-2.5" value="0" readOnly inputMode="decimal" /></label>
