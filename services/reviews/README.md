...
## Customer review editing (REVIEW-1A.11)
Verified purchasers edit only through a dedicated 32-byte opaque edit token. Tokens are SHA-256 hash-only records, tenant-bound, expiry-bound, revocable, and reusable during the original `submittedAt + reviewEditWindowDays` window (default 30 days); each request validates the token and records its use. Reissuing a token revokes prior active tokens. Global editing can be disabled without mutating token rows.

Customers may change only rating, title, and body. Product, order, customer, verified-purchase, moderation, and merchant-reply fields are immutable. A revision records before/after customer content and status for each successful edit. Published and rejected reviews become pending moderation by default (published timestamps are cleared and published aggregates are recalculated); pending reviews stay pending. Merchant replies remain stored and are automatically hidden because storefront queries only show published parents. Customer deletion, unlimited edits, merchant content edits, reply edits, rejection feedback, media revisions, public revision history/edited badges, manual extensions, and bulk token regeneration are deferred.
