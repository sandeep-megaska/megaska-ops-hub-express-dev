...
function orderReward(formData: FormData) {
  if (stringValue(formData, "rewardScope") !== "ORDER") return undefined;
  let tiers: Array<{ id: string; minimumSubtotal: string; maximumSubtotal?: string; percentage: string }> = [];
  try { const parsed = JSON.parse(stringValue(formData, "orderTiers")); if (Array.isArray(parsed)) tiers = parsed.map((tier) => ({ id: String(tier.publicId ?? "").trim(), minimumSubtotal: String(tier.minimumSubtotal ?? "").trim(), ...(String(tier.maximumSubtotal ?? "").trim() ? { maximumSubtotal: String(tier.maximumSubtotal).trim() } : {}), percentage: String(tier.percentage ?? "").trim() })); } catch { /* Canonical validation reports missing tiers. */ }
  return { continuityMode: stringValue(formData, "tierContinuityMode") === "ALLOW_GAPS" ? "allow_gaps" as const : "continuous" as const, tiers };
}
...
export async function changePromotionStatusAction(_prev: PromotionActionState, formData: FormData): Promise<PromotionActionState> { try { const s = await shop(formData); const id = stringValue(formData, "id"); await getPromotionRuleById(s.id, id); const updated = await changePromotionRuleStatus(s.id, id, stringValue(formData, "status") as PromotionRuleStatus, { actor: { actorType: "ADMIN" } }); let extra: Record<string, string | undefined> = { saved: "status" }; try { const publication = await publishPromotionStatusChange(s.id, s.shopDomain, updated); extra = { ...extra, compile: publication.compile, sync: publication.sync }; } catch (e) { extra = { ...extra, publication: "failed", error: publicationErrorParam(e) }; } revalidatePath(`/admin/promotions/${id}`); redirect(actionUrl(formData, `/admin/promotions/${id}`, extra)); } catch (e) { if ((e as Error & { digest?: string }).digest?.startsWith("NEXT_REDIRECT")) throw e; if (e instanceof PromotionRuleNotFoundError) return { ok: false, message: "Promotion rule was not found." }; return state(e); } }
export async function archivePromotionRuleAction(_prev: PromotionActionState, formData: FormData): Promise<PromotionActionState> { try { const s = await shop(formData); const id = stringValue(formData, "id"); const archived = await archivePromotionRule(s.id, id, { actor: { actorType: "ADMIN" } }); let extra: Record<string, string | undefined> = { saved: "status" }; try { const publication = await publishPromotionStatusChange(s.id, s.shopDomain, archived); extra = { ...extra, compile: publication.compile, sync: publication.sync }; } catch (e) { extra = { ...extra, publication: "failed", error: publicationErrorParam(e) }; } revalidatePath("/admin/promotions"); revalidatePath(`/admin/promotions/${id}`); redirect(actionUrl(formData, `/admin/promotions/${id}`, extra)); } catch (e) { if ((e as Error & { digest?: string }).digest?.startsWith("NEXT_REDIRECT")) throw e; return state(e); } }