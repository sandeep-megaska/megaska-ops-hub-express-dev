...
import { publishSavedPromotion } from "./publication-orchestration.server.ts";
...
function publicationErrorParam(error: unknown) { if (error instanceof PromotionCompilationError) return `compile_${error.code}`; return "sync_failed"; }
