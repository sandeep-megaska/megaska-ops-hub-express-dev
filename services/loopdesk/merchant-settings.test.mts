...
import { mergeLoopDeskMerchantSettings, normalizeLoopDeskMerchantSettings, toLoopDeskPublicRuntimeConfig } from './merchant-settings.ts';
...
test('merges partial config without dropping existing settings', () => {
  const current = normalizeLoopDeskMerchantSettings({ branding: { primaryColor: '#123456' }, labels: { viewCartText: 'Bag' } });
  const merged = mergeLoopDeskMerchantSettings(current, { labels: { expressCheckoutText: 'Quick checkout' } });
...
test('falls back invalid colors and drawer modes', () => {
  const settings = normalizeLoopDeskMerchantSettings({ branding: { primaryColor: 'url(javascript:alert(1))' }, cart: { drawerMode: 'broken' } });
  assert.equal(settings.branding.primaryColor, '#111827');
  assert.equal(settings.cart.drawerMode, 'auto');
});

test('public runtime config excludes integration and analytics settings', () => {
  const settings = normalizeLoopDeskMerchantSettings({ integrations: { razorpay: { status: 'configured', keySecret: 'secret' } }, analytics: { enabled: true } });
  const runtime = toLoopDeskPublicRuntimeConfig(settings) as Record<string, unknown>;
...
test('legacy runtime shape remains available', () => {
  const settings = normalizeLoopDeskMerchantSettings({ labels: { checkoutButtonText: 'Checkout now' }, cart: { cartDrawerMode: 'loopdesk' } });
