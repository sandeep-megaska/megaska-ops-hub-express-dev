...
function dbHarness(options: { intent?: any; reservations?: any[]; existing?: any[] } = {}) {
  const state = { created: [] as any[], updated: [] as any[], audits: [] as any[] };
  const db = {
    ...
    codAdvanceIntent: {
      ...
      updateMany: async (args: any) => { state.updated.push(args); return { count: 1 }; },
      create: async (args: any) => { const row = { id: `cod-${state.created.length + 1}`, status: "CREATED", createdAt: new Date(), ...args.data }; state.created.push(row); return row; },
    },
  } as any;
  return { db, state };
}
...
test("identical quote reuses intent", async () => {
  ...
  const h = dbHarness({ intent: base, existing: [{ id: "old", status: "CREATED", pricingFingerprint: fp, settingsVersion: 1, advanceAmountPaise: 12000, codBalanceAmountPaise: 98000, expiresAt: future(), createdAt: new Date() }] });
  const dto = await resolve({ db: h.db, state: h.state });
...
test("changed total creates replacement intent and cancels stale CREATED", async () => {
  const h = dbHarness({ existing: [{ id: "old", status: "CREATED", pricingFingerprint: "old-fp", settingsVersion: 1, advanceAmountPaise: 12000, codBalanceAmountPaise: 97000, expiresAt: future() }] });
...
test("changed Store Credit creates replacement intent", async () => {
  const h = dbHarness({ reservations: [{ id: "res-2", reservedAmount: 1000, createdAt: new Date(), expiresAt: future() }], existing: [{ id: "old", status: "CREATED", pricingFingerprint: "old-fp", settingsVersion: 1, advanceAmountPaise: 12000, codBalanceAmountPaise: 98000, expiresAt: future() }] });
  await resolve({ db: h.db, state: h.state });
  assert.equal(h.state.created.length, 1);
});

test("changed settings version creates replacement intent", async () => {
  const h = dbHarness({ existing: [{ id: "old", status: "CREATED", pricingFingerprint: "old-fp", settingsVersion: 1, advanceAmountPaise: 12000, codBalanceAmountPaise: 98000, expiresAt: future() }] });
...
test("paid intent is never overwritten or cancelled", async () => {
  const h = dbHarness({ existing: [{ id: "paid", status: "ADVANCE_PAID", paidAt: new Date(), pricingFingerprint: "old" }] });
  await resolve({ db: h.db, state: h.state });
...
test("stale CREATED intent is expired when already past expiry", async () => {
  const h = dbHarness({ existing: [{ id: "old", status: "CREATED", pricingFingerprint: "old", expiresAt: past() }] });
  await resolve({ db: h.db, state: h.state });
