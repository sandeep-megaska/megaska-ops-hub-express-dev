/** Customer-facing projection of the canonical order-tier contract. Shopify remains authoritative for pricing. */
type TierProgressState = "BELOW_FIRST_TIER" | "TIER_UNLOCKED" | "HIGHEST_TIER_UNLOCKED";

type TierProgressInputItem = {
  publicId: string;
  position: number;
  minimumSubtotal: string | number;
  maximumSubtotal?: string | number | null;
...
type TierProgressViewModel = {
  enabled: boolean;
  promotionPublicId: string | null;
  currencyCode: string;
  qualifyingSubtotal: number;
  currentTier: TierProgressItem | null;
  nextTier: TierProgressItem | null;
  highestTier: TierProgressItem | null;
  amountToNextTier: number | null;
  progressPercent: number;
  tiers: TierProgressItem[];
  state: TierProgressState;
  message: { eyebrow: string; title: string; description: string };
};

type BuildTierProgressInput = {
  qualifyingSubtotal: number;
  tiers: TierProgressInputItem[];
  currencyCode: string;
  promotionPublicId?: string | null;
...
function disabledTierProgress(input: Partial<BuildTierProgressInput>): TierProgressViewModel {
...
function buildTierProgressViewModel(input: BuildTierProgressInput): TierProgressViewModel {
  const subtotal = Number(input.qualifyingSubtotal);
  if (!Number.isFinite(subtotal) || subtotal < 0 || input.selectionMode !== "highest_eligible" || ["continuous", "allow_gaps"].indexOf(input.continuityMode) < 0 || !/^[A-Z]{3}$/.test(input.currencyCode || "") || !Array.isArray(input.tiers) || !input.tiers.length) return disabledTierProgress(input);
  ...
  for (let matchIndex = 0; matchIndex < tiers.length; matchIndex += 1) {
    ...
    if (subtotal >= candidate.minimumSubtotal && (candidateMaximum === null || subtotal < candidateMaximum)) currentIndex = matchIndex;
  }
  ...
  const state: TierProgressState = highestReached ? "HIGHEST_TIER_UNLOCKED" : current ? "TIER_UNLOCKED" : "BELOW_FIRST_TIER";
...
if (typeof window !== "undefined") (window as unknown as { LoopDeskTierProgress: { buildTierProgressViewModel: typeof buildTierProgressViewModel } }).LoopDeskTierProgress = { buildTierProgressViewModel: buildTierProgressViewModel };

export { buildTierProgressViewModel };
export type { BuildTierProgressInput, TierProgressInputItem, TierProgressItem, TierProgressState, TierProgressViewModel };
