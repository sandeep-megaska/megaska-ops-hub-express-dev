...
const checks = [
  ...
  ['No-advance policy clears stale Partial COD intent', /cod\.codAdvanceIntentId = cod\.requiresAdvance \? \(body\.codAdvanceIntentId \|\| null\) : null/],
  ['No-advance policy clears stale Partial COD customer content', /cod\.customerTitle = cod\.requiresAdvance \?[\s\S]{0,240}cod\.policyText = cod\.requiresAdvance \?/],
  ['Normal COD renders the Place COD Order CTA', /cod\.requiresAdvance \? `Pay \$\{money\(cod\.advanceAmountPaise, cod\.currency\)\} & Confirm COD` : \(method === "COD" \? "Place COD Order"/],
  ['Normal COD does not render Partial COD fields', /if \(!cod\.requiresAdvance\) return `[\s\S]{0,400}Total payable on delivery/],
