...
assert.match(intentRoute, /function couponBaseAmountPaise\(subtotalAmountPaise: number, cartSnapshot: unknown, body: Record<string, unknown>\) \{[\s\S]*validLoopDeskOfferPricing\(snapshot\?\.loopdeskOfferPricing\)[\s\S]*validLoopDeskOfferPricing\(body\.loopdeskOfferPricing\)[\s\S]*handoff \? handoff\.loopdeskOfferAdjustedTotalAmountPaise : subtotalAmountPaise;/, "intent creation should derive the Express Checkout coupon base from the drawer/bag handoff when present");
assert.match(intentRoute, /paiseValues\.totalAmountPaise = Math\.max\([\s\S]*couponBasePaise \+ paiseValues\.shippingAmountPaise \+ paiseValues\.codFeeAmountPaise - paiseValues\.discountAmountPaise/, "intent creation should recalculate payable from the coupon-aware intent pipeline without double-subtracting the drawer offer");
...
assert.match(discountRoute, /resolveExpressCheckoutCoupon[\s\S]*calculateExpressCheckoutDiscount/, "discount route should resolve coupon definitions before generic calculation");
assert.match(discountRoute, /const totalAmountPaise = recalculateTotal\(editable\.intent, calculatedDiscount\.discountAmountPaise\);[\s\S]*discountAmountPaise: calculatedDiscount\.discountAmountPaise,[\s\S]*totalAmountPaise/, "applying a coupon should persist the non-zero discount and final payable together");
