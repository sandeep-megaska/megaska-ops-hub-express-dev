...
assert.equal(viewModel.offerCards[0].promotionalUnitPrice, viewModel.cartLines.find((line) => line.isPromotionAdjusted).displayUnitPrice, "cart line percentage display matches offer card value");
for (const [label, discount] of [["fixed_amount", { type: "fixed_amount", value: 50 }], ["fixed_price", { type: "fixed_price", value: 150 }]]) {
  ...
  assert.equal(model.offerCards[0].promotionalUnitPrice, model.cartLines.find((line) => line.isPromotionAdjusted).displayUnitPrice, `cart line ${label} display matches offer card value`);
}
