...
assert.match(js, /moneyMinor/);
assert.match(js, /Available balance/);
assert.match(js, /No Store Credit activity yet/);
assert.match(js, /storeCreditDirection\(t\)/);
assert.match(js, /storeCreditReason\(t\)/);
assert.match(js, /firstText\(t\?\.reason, t\?\.note, t\?\.description, t\?\.transactionType, t\?\.sourceType/);
...
assert.match(js, /Request cancellation before the order is shipped\./);
...
assert.match(js, /fetch\(cancellationApiUrl\(\)/);
assert.doesNotMatch(js, /fetch\([^)]*requests\/(exchange|issue)/s);
