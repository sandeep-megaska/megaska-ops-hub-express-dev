(function () {
  ...
  function triggerMatches(trigger, cart) {
    ...
    if (type === "cart_subtotal_gte" || type === "cart_subtotal_min") return rawCartSubtotal(cart) >= Number(trigger.subtotalGte || value || 0);
  ...
  function parseMoneyAmount(value) { var raw = String(value || "").trim(); if (!raw) return 0; var match = raw.match(/(-?\d+(?:[,.]\d+)?)/); if (!match) return 0; var amount = Number(match[1].replace(/,/g, "")); return Number.isFinite(amount) ? amount : 0; }
  function parseMoneyCents(value) { var amount = parseMoneyAmount(value); return amount > 0 ? Math.round(amount * 100) : 0; }
  function centsToAmount(cents) { return Math.round((Math.max(0, Number(cents) || 0) / 100) * 100) / 100; }
  function resolveRewardVariantPrice(rule, reward, display) {
    reward = plain(reward) ? reward : {}; display = plain(display) ? display : {}; var product = plain(reward.product) ? reward.product : {};
    var paths = [
      { source: "reward.product.variantPrice", value: product.variantPrice },
      { source: "reward.variantPrice", value: reward.variantPrice },
      { source: "rewardProductVariantPrice", value: rule && rule.rewardProductVariantPrice },
      { source: "legacy_compare", value: display.comparePriceDisplay }
    ];
    for (var i = 0; i < paths.length; i += 1) { var amount = parseMoneyAmount(paths[i].value); if (amount > 0) return { amount: amount, cents: Math.round(amount * 100), source: paths[i].source }; }
    return { amount: 0, cents: 0, source: "unavailable" };
  }
  function unitFromLine(item) { var qty = Math.max(1, Number(item && item.quantity) || 1); return Math.round(Number(item && (item.original_line_price || item.line_price || item.final_line_price) || 0) / qty); }
  ...
  function buildPromotionViewModel(input) {
    input = plain(input) ? input : {}; var cart = input.cart || { items: [] }; var rules = Array.isArray(input.rules) ? input.rules : []; var currency = input.currency || cart.currency || null; var subtotal = rawCartSubtotal(cart); var vm = { hasPromotion: false, currency: currency, offerCards: [], cartLines: [], totals: { shopifySubtotal: subtotal, promotionDiscountTotal: 0, estimatedAfterOffer: subtotal, hasEstimatedTotal: false, subtotalNote: "Subtotal is the Shopify cart subtotal before display-only offers.", payableNote: "Final discount is applied at checkout." } };
    rules.forEach(function (rule) { var reward = plain(rule.reward) ? rule.reward : {}; var display = plain(rule.display) ? rule.display : {}; var product = plain(reward.product) ? reward.product : {}; var gid = resolveVariantGid(reward); var configuredPrice = resolveRewardVariantPrice(rule, reward, display); var overrideCents = Number(input.rewardUnitPrices && input.rewardUnitPrices[normalizeShopifyId(gid)] || 0) || 0; var originalCents = overrideCents > 0 ? overrideCents : configuredPrice.cents; var priceSource = overrideCents > 0 ? "reward.product.variantPrice" : configuredPrice.source; var qty = Math.max(1, Number(reward.quantity) || 1); var resolved = calculatePromotion({ cart: cart, rule: rule, rewardVariantGid: gid, rewardUnitPrice: originalCents, rewardQuantity: qty }); var originalAmount = configuredPrice.amount || centsToAmount(originalCents); var promotionalAmount = resolved.promotionalUnit === null ? null : centsToAmount(resolved.promotionalUnit); var discountAmount = centsToAmount(resolved.discountPerUnit); var override = String(display.offerPriceDisplay || "").trim(); var priceText = resolved.eligible ? money(resolved.promotionalUnit, currency) : override; vm.offerCards.push({ ruleId: rule.id || "", heading: display.heading || rule.name || "Special offer", badge: display.badge || "", title: product.title || rule.name || "Offer product", image: display.imageOverrideUrl || product.imageUrl || product.image || "", ctaLabel: display.ctaLabel || "Add offer", rewardVariantId: gid, originalUnitPrice: originalAmount, promotionalUnitPrice: promotionalAmount, discountPerUnit: discountAmount, discountType: resolved.discountType, discountValue: resolved.discountValue, eligibleQuantity: resolved.eligibleQuantity, priceText: priceText, originalPriceText: originalCents > 0 ? money(originalCents, currency) : String(display.comparePriceDisplay || ""), benefitText: resolved.eligible ? "Get it for " + money(resolved.promotionalUnit, currency) + ", Usually " + money(originalCents, currency) : "", labelText: display.badge || "Offer applied", noteText: resolved.eligible ? "Discount applied at checkout" : "", canShowPriceBenefit: Boolean(resolved.eligible && originalCents > 0), priceSource: priceSource }); });
  ...
  window.LoopDeskPromotionPricing = { normalizeShopifyId: normalizeShopifyId, sameShopifyId: sameId, resolvePromotionDisplayPricing: resolvePromotionDisplayPricing, buildPromotionViewModel: buildPromotionViewModel, summarizeCart: summarizeCart, rawCartSubtotal: rawCartSubtotal };
})();
