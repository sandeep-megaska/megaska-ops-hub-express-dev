(function () {
  ...
  var stats = { cartAdds: 0, opens: 0, otpOpens: 0, failures: 0, observerStarts: 0 };
  ...
  function isBuyNowControl(target) {
    var control = closest(target, buyNowSelector);
    if (!control || isThemeEditorControl(control) || isNativeCheckoutControl(control)) return null;
    if (matches(control, '[name="add"], [data-add-to-cart], [data-wishlist], [aria-label*="wishlist" i], [class*="wishlist" i]')) return null;
    return control;
  }
  ...
  async function handle(event) {
    ...
    try {
      await addToCart(data); notifyCart();
      if (hasToken()) openCheckout(); else { setResume(); if (!openOtp()) openCheckout(); }
    } catch (err) { stats.failures += 1; announce(err && err.message || "We could not prepare checkout. Please try again."); restore(control, old); inflight.delete(control); }
  }
  function process(root) { root = root || document; var nodes = matches(root, buyNowSelector) ? [root] : Array.prototype.slice.call(root.querySelectorAll ? root.querySelectorAll(buyNowSelector) : []); nodes.forEach(function (n) { n.dataset[PROCESSED] = "true"; }); }
  ...
  window[API_NAME] = { selectors: selectors.slice(), status: function () { return { loaded: true, observerStarts: stats.observerStarts, cartAdds: stats.cartAdds, opens: stats.opens, otpOpens: stats.otpOpens, failures: stats.failures }; }, _test: { resolveForm: resolveForm, isBuyNowControl: isBuyNowControl, validate: validate, process: process } };
})();
