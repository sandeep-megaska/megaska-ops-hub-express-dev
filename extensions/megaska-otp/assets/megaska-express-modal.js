...
function buildBufferedEta(rawEta) {
  ...
  function expressBasePayablePaise() {
    return Math.max(0, Number(state.intent?.totalAmountPaise || 0));
  }
  function expressFinalPayablePaise() {
    return Math.max(0, expressBasePayablePaise() - expressPromotionDiscountPaise());
  }
  function expressPromotionSummary(intent) {
    const promotionDiscount = expressPromotionDiscountPaise();
    return promotionDiscount > 0 ? `<p><span>LoopDesk offer discount</span><strong>-${money(promotionDiscount, intent.currency)}</strong></p>` : "";
  }
  ...
  function payableAmount(method) {
    return Math.max(0, remainingBasePayablePaise());
  }
...
function renderStoreCreditOrderPanel() {
  ...
  async function ensureRazorpayOrder() {
    ...
    const promise = (async () => {
      const data = await apiFetch(`/express/checkout/intents/${encodeURIComponent(intentId)}/razorpay-order`, { method: "POST", body: { amountPaise: payableAmount("PREPAID"), promotionDiscountPaise: expressPromotionDiscountPaise() } });
      const checkout = Object.assign({}, data.checkout || {}, { amountPaise: payableAmount("PREPAID") });
  ...
  async function createOrder() {
    ...
    const data = await apiFetch(`/express/checkout/intents/${encodeURIComponent(currentIntentId)}/order`, { method: "POST", body: { amountPaise: payableAmount(paymentMethod), promotionDiscountPaise: expressPromotionDiscountPaise() } });
  ...
  function inlinePayload(method, data, checkout) {
    const basePayload = { order_id: checkout.razorpayOrderId, amount: payableAmount("PREPAID"), currency: checkout.currency || "INR", email: checkout.customer?.email || state.customer?.email || "", contact: checkout.customer?.contact || state.intent?.phoneSnapshot || "" };
