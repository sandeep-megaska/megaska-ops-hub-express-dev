(function () {
 ...
 const PAGE_FALLBACK_URL = "/apps/megaska/checkout";
  ...
  function discountSummary(intent) { const discount = Array.isArray(intent?.discounts) ? intent.discounts[0] : null; if (!discount || !Number(intent?.discountAmountPaise || 0)) return ""; const raw = discount.rawShopifyPayload || {}; const code = discount.code || raw.discountCode || discount.title || "Discount"; const off = raw.discountType === "PERCENTAGE" && raw.discountValue ? ` — ${escapeHtml(raw.discountValue)}% OFF` : ""; return `<p><span>${escapeHtml(code)} applied${off}<br><small>You saved ${money(intent.discountAmountPaise, intent.currency)}</small></span><strong>- ${money(intent.discountAmountPaise, intent.currency)}</strong></p>`; }
