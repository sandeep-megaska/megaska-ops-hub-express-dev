(function () {
  ...
  function renderStoreCreditBlock(intent, priceHydrating) {
    ...
    return `<section class="megaska-express-stack megaska-express-store-credit">${credit.loading ? `<div class="megaska-express-store-credit-card"><div class="megaska-express-store-credit-head"><h3>Megaska Store Credit</h3><span class="megaska-express-store-credit-pill">Loading</span></div><p class="megaska-express-store-credit-note">Checking available store credit...</p></div>` : `<div class="megaska-express-store-credit-card${appliedPaise > 0 ? " is-applied" : ""}"><div class="megaska-express-store-credit-head"><h3>Megaska Store Credit</h3><label class="megaska-express-store-credit-toggle" aria-label="${appliedPaise > 0 ? "Remove Megaska Store Credit" : "Apply Megaska Store Credit"}"><input type="checkbox" ${appliedPaise > 0 ? "checked" : ""} ${state.busy ? "disabled" : ""} data-express-action="${appliedPaise > 0 ? "release-store-credit" : "apply-store-credit"}"><span class="megaska-express-store-credit-switch" aria-hidden="true"></span></label></div><div class="megaska-express-store-credit-body"><p><span>Available</span><strong>${money(Math.round(Number(credit.availableAmount || 0) * 100), credit.currency || intent.currency)}</strong></p>${appliedPaise > 0 ? `<span class="megaska-express-store-credit-pill">Applied</span>` : ""}</div>${appliedPaise > 0 ? `<div class="megaska-express-store-credit-breakdown"><p><span>Using</span><strong>${money(appliedPaise, credit.currency || intent.currency)}</strong></p><p><span>Remaining payment</span><strong>${priceHydrating ? "Calculating..." : money(remainingPaise, credit.currency || intent.currency)}</strong></p></div>` : ""}</div>`}${credit.error ? `<p class="megaska-otp-error">${escapeHtml(credit.error)}</p>` : ""}</section>`;
  }
