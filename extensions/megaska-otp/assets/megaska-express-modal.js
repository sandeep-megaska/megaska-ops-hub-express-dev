(function () {
 ...
 const PAGE_FALLBACK_URL = "/apps/megaska/checkout";
  ...
  async function ensurePaymentMethod(method) { if (state.intent?.selectedPaymentMethod !== method) await apiFetch(`/express/checkout/intents/${encodeURIComponent(state.intent.id)}/payment-method`, { method: "POST", body: { method } }); await refreshIntent(); if (state.intent?.selectedPaymentMethod !== method) throw new Error(`Unable to persist ${method} payment method.`); }

  async function onChange(event) {
    if (!event.target.matches('input[name="paymentMethod"]')) return;
    try { state.busy = true; render(); await ensurePaymentMethod(event.target.value); state.busy = false; render(); } catch (error) { state.busy = false; state.error = error instanceof Error ? error.message : "Unable to update payment method."; render(); }
  }
