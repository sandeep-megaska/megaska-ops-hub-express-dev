(function () {
  ...
  function ensureModal() {
    ...
    modal.innerHTML = `<div class="megaska-otp-backdrop"></div><div class="megaska-otp-dialog megaska-express-dialog" role="dialog" aria-modal="true" aria-labelledby="megaska-express-title"><button class="megaska-otp-close" type="button" data-express-close aria-label="Close">&times;</button><header class="megaska-express-modal-header" data-express-shell-header></header><div class="megaska-otp-flow megaska-express-scroll-area" data-express-shell-body><div data-express-root></div></div><footer class="megaska-express-sticky-cta" data-express-shell-footer></footer></div>`;
  ...
  function renderShellHeader() {
    const header = ensureModal().querySelector("[data-express-shell-header]");
    if (!header) return;
    header.innerHTML = `<div class="megaska-express-logo">${logoMarkup()}</div><div class="megaska-express-heading"><p class="megaska-otp-step-subtitle">Secure Checkout</p><h2 id="megaska-express-title" class="megaska-otp-step-title">Express checkout</h2></div>`;
  }

  function renderShellFooter(totalAmount) {
    const footer = ensureModal().querySelector("[data-express-shell-footer]");
    if (!footer) return;
    footer.innerHTML = `<div class="megaska-express-sticky-trust"><p><span>🔒</span><strong>100% Secure Payments</strong></p><p><span>🛡</span><strong>Trusted & Reliable</strong></p></div><div class="megaska-express-sticky-main"><div><span>Total Payable</span><strong data-express-total-payable>${totalAmount}</strong></div></div>`;
  }
  ...
  function renderCheckout(root) {
    ...
    renderShellHeader();
    renderShellFooter(totalAmount);
    root.innerHTML = `${state.error ? `<p class="megaska-otp-error">${escapeHtml(state.error)}</p>` : ""}<div class="megaska-express-progress"><span>Address</span><span>Coupon</span><span>Payment</span></div><section class="megaska-express-summary"><h3>Order summary</h3>${rows || `<p class="megaska-otp-step-subtitle">${state.hydration.cart === "loading" ? "Loading cart summary..." : "Cart details unavailable."}</p>`}${extraCount ? `<p class="megaska-otp-step-subtitle">+ ${extraCount} more item${extraCount > 1 ? "s" : ""}</p>` : ""}<div class="megaska-express-totals"><p><span>Subtotal</span><strong>${priceHydrating ? "Calculating..." : money(intent.subtotalAmountPaise, intent.currency)}</strong></p>${discountSummary(intent)}<p><span>Delivery</span><strong>${Number(intent.shippingAmountPaise || 0) ? money(intent.shippingAmountPaise, intent.currency) : "Free"}</strong></p><p class="megaska-express-total"><span>Total</span><strong>${totalAmount}</strong></p></div></section>${addressMarkup}<form data-express-form="discount" class="megaska-express-stack"><h3>Have a coupon?</h3><div class="megaska-express-inline"><input name="code" value="${escapeHtml(state.discountCode)}" placeholder="Enter coupon code"><button type="submit" ${!state.intent?.id || state.busy ? "disabled" : ""}>Apply</button></div>${discountChip}</form>${storeCreditBlock}<section class="megaska-express-stack megaska-express-payment${storeCreditFullyCovers ? " megaska-express-payment--store-credit-only" : ""}" data-express-payment-section>${storeCreditFullyCovers ? renderStoreCreditOrderPanel() : (state.inlinePaymentMode ? renderInlinePaymentPanel(selectedDisplayMethod) : renderPaymentMethodList())}</section>`;
  ...
  function renderStoreCreditBlock(intent, priceHydrating) {
    ...
    const currency = credit.currency || intent.currency;
    const availableLabel = money(Math.round(Number(credit.availableAmount || 0) * 100), currency);
    const usingLabel = money(appliedPaise, currency);
    const remainingLabel = priceHydrating ? "Calculating..." : money(remainingPaise, currency);
    const fullyCovered = !priceHydrating && appliedPaise > 0 && remainingPaise <= 0;
    const partialNote = appliedPaise > 0 && !fullyCovered ? "Selected payment method will be used only for the remaining amount." : "";
    return `<section class="megaska-express-stack megaska-express-store-credit${appliedPaise > 0 ? " is-applied" : ""}"><div class="megaska-express-store-credit-head"><h3>Megaska Store Credit</h3><strong>Available: ${availableLabel}</strong></div>${credit.loading ? `<p class="megaska-otp-step-subtitle">Loading Megaska Store Credit...</p>` : `<div class="megaska-express-store-credit-card"><label class="megaska-express-store-credit-toggle"><input type="checkbox" ${appliedPaise > 0 ? "checked" : ""} ${state.busy ? "disabled" : ""} data-express-action="${appliedPaise > 0 ? "release-store-credit" : "apply-store-credit"}"><span class="megaska-express-store-credit-check" aria-hidden="true">${appliedPaise > 0 ? "✓" : ""}</span><span class="megaska-express-store-credit-copy"><strong>Use Store Credit</strong><small>${appliedPaise > 0 ? "Applied to this checkout" : "Apply available balance to this order"}</small></span></label><div class="megaska-express-store-credit-breakdown"><p><span>Using:</span><strong>${usingLabel}</strong></p><p><span>Remaining:</span><strong>${remainingLabel}</strong></p></div>${fullyCovered ? `<p class="megaska-express-store-credit-note is-complete">No additional payment required.</p>` : partialNote ? `<p class="megaska-express-store-credit-note">${escapeHtml(partialNote)}</p>` : ""}</div>`}${credit.error ? `<p class="megaska-otp-error">${escapeHtml(credit.error)}</p>` : ""}</section>`;
  }
  ...
  function renderStoreCreditOrderPanel() {
    ...
    return `<h3>Payment</h3><div class="megaska-express-inline-panel megaska-express-store-credit-order-panel"><div class="megaska-express-inline-fields"><p><strong>No additional payment required</strong></p><p><strong>Remaining Payment: ₹0.00</strong></p><p class="megaska-express-secure-note">Megaska Store Credit covers this order.</p></div><button class="megaska-otp-primary-btn" type="button" data-express-action="store-credit-order" ${busy ? "disabled" : ""}>${state.orderSubmitting ? "Placing order..." : "Place Order"}</button></div>`;
  }
  ...
  function renderInlinePaymentPanel(method) {
    ...
    if (method === "COD") fields = `<div class="megaska-express-cod-confirm"><strong>Confirm Cash on Delivery</strong><p>Pay only the remaining payable amount when your order is delivered.</p><p>Total payable on delivery: <b>${escapeHtml(totalLabel)}</b></p></div>`;
  ...
  async function createOrder() {
    ...
    console.info("[Megaska Express] checkout_order_response", { currentIntentId, returnedIntentId: checkoutResponseIntentId(data), returnedOrderName: checkoutResponseOrderName(data) || null, returnedOrderId: checkoutResponseOrderId(data) || null, selectedPaymentMethod: responsePaymentMethod, freshCompletion: data?.freshCompletion === true, completionSource: data?.completionSource || null });
