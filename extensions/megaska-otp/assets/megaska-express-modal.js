...
function buildBufferedEta(rawEta) {
  ...
  function loopDeskRuntimeConfig() { return window.LoopDeskConfig && typeof window.LoopDeskConfig === "object" ? window.LoopDeskConfig : {}; }
  function shopLabel() {
    const config = loopDeskRuntimeConfig();
    return String(config.branding?.merchantName || config.branding?.storeName || "Megaska");
  }
  function logoUrl() {
    const config = loopDeskRuntimeConfig();
    const sources = [
  ...
  function logoMarkup() {
    const src = logoUrl();
    if (src) return `<img class="megaska-express-logo-img" src="${escapeHtml(src)}" alt="${escapeHtml(shopLabel())}" loading="lazy">`;
    return `<span class="megaska-express-logo-text"><strong>${escapeHtml(shopLabel())}</strong><small>Express checkout</small></span>`;
  }
